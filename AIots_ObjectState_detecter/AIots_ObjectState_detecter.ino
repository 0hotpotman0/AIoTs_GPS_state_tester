...
#include <SPI.h>
#include <WT_3Asix_inferencing.h>
//#include <accel_gesture_spectral_inference.h>
#include"LIS3DHTR.h"
...
void display_EVI() {
  if (init_ui == 0) {
    ...
    tft.drawRoundRect(0, 0, 320, 30, 6, TFT_YELLOW);
    tft.setTextColor(TFT_WHITE);                //Setting text color
    tft.drawString("GPS & WT State", 45, 5); //Drawing text string
...
void refreshGpsInfo() {
  ...
  if (init_ui == 0) {
    ...
    tft.drawString("Predict Result:", 5, 88, 1); // Print the test text in the custom font
    tft.drawRoundRect(185, 75, 100, 40, 5, TFT_WHITE);

    tft.setFreeFont(&FreeSansBoldOblique9pt7b);
    //    tft.drawString("%", 160, 85, 1);
  ...
  //    //    tft.fillRect(90, 88, 25, 25, TFT_BLACK);
  //    //    tft.drawNumber(25, 90, 88, GFXFF);    // temp  MARK
  //    P_temp = temp;
  //  }
  //
  //  if (humi != P_humi || init_ui == 0) {
  //    tft.fillRect(210, 88, 80, 25, TFT_BLACK);
  //    tft.drawString(result1, 200, 88, GFXFF);   // humi MARK
  //    P_humi = humi;
  //  }
...
void loop(void) {
  ...
  if (tTime > 10 * 1000 && switch_UI == 0) {
    ...
    Stop = result.classification[0].value;
    Serial.print("Turn: ");
    Serial.println(result.classification[1].value);
    Turn = result.classification[1].value;
    Serial.print("Wave: ");
    Serial.println(result.classification[2].value);
    Wave = result.classification[2].value;
    Serial.println("##################################################");
    if (result.classification[0].value > 0.7) {
      tft.fillRect(200, 88, 60, 25, TFT_BLACK);
      tft.drawString("Stop", 200, 88, GFXFF);
      Stop = result.classification[0].value;
    }
    if (result.classification[1].value > 0.7) {
      tft.fillRect(200, 88, 60, 25, TFT_BLACK);
      tft.drawString("Turn", 200, 88, GFXFF);
      Turn = result.classification[1].value;
    }
    if (result.classification[2].value > 0.7) {
      tft.fillRect(200, 88, 60, 25, TFT_BLACK);
      tft.drawString("Wave", 200, 88, GFXFF);
      Wave = result.classification[2].value;
    }
    GpsSerialInit();
    ...
    case MODULE_IDLE:
      if (module_AT_Cmd <= AT_PORT) {
        ...
        if (cTime > 10 * 1000)
        {
          ...
          sprintf(cmd, "AT+CMSGHEX=\"%04X%04X%04X\"\r\n", (int)(Stop * 10000), (int)(Turn * 10000), (int)(Wave * 10000));
